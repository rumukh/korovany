...
export function applyStylizedShader(
  ...
  material.customProgramCacheKey = () => features?.enhanced
    ? `${STYLIZED_PROGRAM_KEY}:${artShaderKey(features)}:${AFFINE_SKIN_NORMAL_REVISION}` : STYLIZED_PROGRAM_KEY
...
export function applyOutlineShader(
  ...
  material.customProgramCacheKey = () =>
    `${OUTLINE_PROGRAM_KEY}:${smooth ? 'smooth' : 'flat'}${features?.enhanced ? `:${artShaderKey(features)}:${AFFINE_SKIN_NORMAL_REVISION}` : ''}`
  material.needsUpdate = true
}
