...
test('every faceted displacement site in the world runtime opts in', () => {
  // Guarding the invariant where it is *relied on*, not only where it lives. The
  // behavioural tests above all pass with `seamless` deleted from both call sites.
  const source = readFileSync(
    new URL('../src/game/world/GeneratedWorldRuntime.ts', import.meta.url),
    'utf8',
  )
  ...
  const callSites: { target: string; body: string; at: number }[] = []
  ...
  for (let at = source.indexOf(marker); at !== -1; at = source.indexOf(marker, at + 1)) {
    ...
    if (comma === -1) continue
    callSites.push({ target: argumentText.slice(0, comma).trim(), body: argumentText, at })
  }

  assert.ok(
    callSites.length >= 3,
    `expected to find the displacement sites, found ${callSites.length}`,
  )

  // Derived rather than listed: any site built on a faceted source has to opt in. The
  // fort boulder is the one exception and it is exempt by measurement, not by opinion —
  // `IcosahedronGeometry` at detail 1 carries radial normals and tears zero edges.
  const exempt = /IcosahedronGeometry\([^)]*,\s*[1-9]/
  let checked = 0
  for (const site of callSites) {
    if (exempt.test(source.slice(Math.max(0, site.at - 400), site.at))) continue
    checked += 1
    assert.match(
      site.body,
      /seamless:\s*true/,
      `displaceGeometry(${site.target}, …) builds on a faceted source and must pass seamless: true`,
    )
  }
  assert.ok(checked > 0, 'every site was exempted, so this test asserted nothing')
})
