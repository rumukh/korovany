...
- **The population question was put to every claim here and never once to the population of
  claims.** A sibling measured their own branch at the end: **29 commits, 2 merges, and of
  the remaining 27 — zero code-only, 18 docs-only, 9 mixed**, with the last eight consecutive
  commits touching no code at all. Both branches ended the same way.
  *"There is still yield here"* therefore stopped being evidence that here is where the work
  belongs. **Every round's artefact is the next round's subject**, so the population of
  claims is *regenerated by the act of examining it* and its defect rate cannot reach zero.
  **That every exchange produced a real finding is what a fixed point looks like from the
  inside**, not what a productive seam looks like.
  So the stopping rule has to come from outside the process, because the process cannot
  generate one:
  > **An exchange whose only product is an audit of the previous exchange has met the
  > terminating condition.**
  **And the entry stating that carried two fresh defects, one of which is a new class.** The
  first: two expiring sentences, of which the guard caught the one containing a phrase it
  knows and passed a bare commit count with a moving referent — one authoring defect, two
  instances, and the instrument sees only the one that says a word from its list.
  The second is the one worth keeping. The split shipped as *twenty* docs-only; it is
  **eighteen**:
  ```
  buckets: code / docs / both      no arm for "neither"
  0 + 18 + 9 = 27                  published under a population of 29
  the gap raised nothing
  ```
  > **A partition with no residual bucket cannot report that it dropped anything.**
  Not a filter returning empty for a real object, but a **complete-looking decomposition that
  is short and says so nowhere** — and the fix is not a threshold, which is what the rest of
  this file is made of, but an **equality: the parts must sum to the population.**
  **The same defect was live in `verify.mjs` and is now closed there.** Its sweep reported
  `prose` against `allAdded` with the fenced remainder going nowhere, and a file skipped by
  any guard contributed to the total and to neither bucket, silently. It now prints
  `added = prose + fenced` and **fails on a non-zero residual**, with two controls red under
  mutation. Found by applying a sibling's finding at this site rather than recording it —
  which is the one remedy this section has for rules that do not transfer.
